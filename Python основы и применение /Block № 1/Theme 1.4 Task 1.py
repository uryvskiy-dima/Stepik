...
но в пространстве global, внутри которого находится пространство foo, она объявлена. Аналогично, результатом запроса get bar b
будет являться foo, а результатом работы get bar a будет являться bar.
...
def recursion_get(namespace_find, value_find):
    if value_find in namespace[namespace_find]['values']:
        return namespace_find
    else:
        if namespace[namespace_find]['parent'] != 'None':
            return recursion_get(namespace[namespace_find]['parent'], value_find)


count_command = int(input())
namespace = {'global': {'parent': 'None', 'values': []}}
for i in range(count_command):
    command, parameter_one, parameter_two = input().split()
    if command == 'add':
        namespace[parameter_one]['values'].append(parameter_two)
    elif command == 'create':
        namespace[parameter_one] = {'parent': parameter_two, 'values': []}
    else:
        print(recursion_get(parameter_one, parameter_two))
