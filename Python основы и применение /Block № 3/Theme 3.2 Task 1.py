...
string, string_a, string_b = input(), input(), input()
count = 0
if string_a in string_b and string_a in string:  # a = b  и  a  есть в s
    print("Impossible")
else:
    while string_a in string:
        string = string.replace(string_a, string_b)
        count += 1
        if count > 1000:  # условие count > 1000 операций
            count = "Impossible"
            break
    print(count)
