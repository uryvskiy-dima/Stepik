...
к API необходимо зарегистрироваться в проекте, создать свое приложение, и получить уникальный ключ (или токен), и
в дальнейшем все запросы к API осуществляются при помощи этого ключа.
Чтобы начать работу с API проекта Artsy, вам необходимо пройти на стартовую страницу документации к
API https://developers.artsy.net/start
и выполнить необходимые шаги, а именно зарегистрироваться, создать приложение, и получить пару идентификаторов Client Id
и Client Secret.
...
на сайте используется кодировка UTF-8, что может привести к ошибке при попытке записать в файл имя с необычными символами.
...
def get_token():
    client_id = '40c49d5377087bfe218b'
    client_secret = '3addd16b9c9f1651e04fee74c6e2ea0f'

    # инициируем запрос на получение токена
    response_token = requests.post("https://api.artsy.net/api/tokens/xapp_token",
                      data={
                          "client_id": client_id,
                          "client_secret": client_secret
                      })
    ...
    # разбираем ответ сервера
    js = json.loads(response_token.text)
...
with open("Test.txt") as file_data:
    ...
    for name_value in read:
        if name_value == '\n':
            break
        r = requests.get(f"https://api.artsy.net/api/artists/{name_value.rstrip()}", headers=headers)
        r.encoding = 'utf-8'  # ЭТО ОЧЕНЬ ВАЖНОЕ ДОПОЛНЕНИЕ !!!
        # разбираем ответ сервера
        j = json.loads(r.text)
        dictionary[j['sortable_name']] = j['birthday']


for name in sorted(dictionary.items(), key=lambda x: (x[1], x[0])):  # сортирует словарь
    print(name[0])
